...
const Search = {
  ...
  async search(query, offset = 0) {
    ...
    try {
      const data = await API.get(`/api/reception/patients/search?q=${encodeURIComponent(query)}&limit=10&offset=${offset}`);
    ...
    this.currentResults.forEach(patient => {
      ...
      card.innerHTML = `
        <div class="patient-card-header">
          ...
          <button class="btn-edit" onclick="Search.viewPatient(${patient.id})">
            ✏️ Edit
          </button>
        </div>
